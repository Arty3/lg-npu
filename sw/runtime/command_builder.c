...
void npu_cmd_build_conv(struct npu_cmd_desc* desc,
                        const struct npu_conv_params* p)
{
    cmd_clear(desc);
    desc->words[0]  = (uint32_t)NPU_OP_CONV;
...
void npu_cmd_build_gemm(struct npu_cmd_desc* desc,
                        const struct npu_gemm_params* p)
{
    cmd_clear(desc);
    desc->words[0]  = (uint32_t)NPU_OP_GEMM;
...
void npu_cmd_build_softmax(struct npu_cmd_desc* desc,
                           const struct npu_softmax_params* p)
{
    cmd_clear(desc);
    desc->words[0] = (uint32_t)NPU_OP_SOFTMAX;
...
void npu_cmd_build_vec(struct npu_cmd_desc* desc,
                       const struct npu_vec_params* p)
{
    cmd_clear(desc);
    desc->words[0]  = (uint32_t)NPU_OP_VEC;
    desc->words[1]  = p->a_addr;
    desc->words[2]  = p->out_addr;
    desc->words[3]  = p->b_addr;
...
void npu_cmd_build_lnorm(struct npu_cmd_desc* desc,
                         const struct npu_lnorm_params* p)
{
    cmd_clear(desc);
    desc->words[0]  = (uint32_t)NPU_OP_LNORM;
    desc->words[1]  = p->in_addr;
    desc->words[2]  = p->out_addr;
    desc->words[5]  = p->num_rows;
    desc->words[6]  = p->row_len;
...
void npu_cmd_build_pool(struct npu_cmd_desc* desc,
                        const struct npu_pool_params* p)
{
    cmd_clear(desc);
    desc->words[0]  = (uint32_t)NPU_OP_POOL;
    desc->words[1]  = p->in_addr;
    desc->words[2]  = p->out_addr;
    desc->words[5]  = p->in_h;
    desc->words[6]  = p->in_w;
